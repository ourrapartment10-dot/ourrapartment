...
export async function deleteMultipleFromS3(keys: string[]): Promise<boolean[]> {
  const results = await Promise.all(keys.map((key) => deleteFromS3(key)));
  return results;
}
 ...
 * @returns Array of results (true for success, false for failure)
 */
export async function deleteMultipleFromS3ByUrl(
  urls: string[]
): Promise<boolean[]> {
  const results = await Promise.all(urls.map((url) => deleteFromS3ByUrl(url)));
  return results;
}
