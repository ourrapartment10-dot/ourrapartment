...
export function useS3SignedUrls(
  ...
  useEffect(() => {
    ...
    const fetchUrls = async () => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : 'Failed to fetch signed URLs'
        );
      } finally {
        setLoading(false);
      }
  ...
  return {
    ...
    getSignedUrl: useCallback(
      (url: string) => signedUrls[url] || null,
      [signedUrls]
    ),
  };
}
...
export function useS3Image(imageUrl: string | null | undefined) {
  const { signedUrls, loading, error, getSignedUrl } =
    useS3SignedUrls(imageUrl);
