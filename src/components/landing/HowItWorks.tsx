      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-4">
          {steps.map((item, index) => (
            ...
            >
              <div className="bg-primary/10 text-primary group-hover:bg-primary mx-auto mb-8 flex h-24 w-24 items-center justify-center rounded-3xl shadow-sm transition-all duration-300 group-hover:scale-110 group-hover:text-white group-hover:shadow-2xl md:bg-gray-50 md:text-gray-300">
                {item.step}
              </div>
              ...
              {index < steps.length - 1 && (
                <div className="absolute top-12 left-1/2 -z-10 hidden h-0.5 w-full translate-x-1/2 -translate-y-1/2 bg-gradient-to-r from-gray-100 to-gray-200 lg:block" />
              )}
            </motion.div>
          ))}
        </div>
