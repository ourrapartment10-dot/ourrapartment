...
export default function AnnouncementFeed({ initialPosts, user }: { initialPosts: any[], user: any }) {
    const [posts, setPosts] = useState(initialPosts);
    const [loading, setLoading] = useState(false);
    const [signedImageUrls, setSignedImageUrls] = useState<Record<string, string | null>>({});
    const [loadingImages, setLoadingImages] = useState(true);
    const imageUrlCache = useRef<Record<string, { url: string | null; expiresAt: number }>>({});
    ...
    useEffect(() => {
        const fetchSignedUrls = async () => {
            ...
            for (const post of posts) {
                if (imageUrlCache.current[post.id] && imageUrlCache.current[post.id].expiresAt > now) {
            ...
            for (const post of posts) {
                ...
                if (post.imageUrl) {
                    try {
                        ...
                        console.error(`Error fetching signed URL for post ${post.id}:`, error);
                        urls[post.id] = null;
                    }
                } else {
                    urls[post.id] = null;
    ...
    if (posts.length === 0) {
        return (
            <div className="text-center py-20">
                ...
                <p className="text-gray-500">Stay tuned for updates from the administration.</p>
            </div>
        );
    }
