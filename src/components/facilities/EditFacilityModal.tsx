...
export default function EditFacilityModal({ isOpen, onClose, onSuccess, facility }: EditFacilityModalProps) {
    ...
    // Use S3 upload hook
    const { upload, uploading, uploadedUrl, reset: resetUpload, setUrl } = useS3Upload("facilities");
    ...
    useEffect(() => {
        if (facility && isOpen) {
            ...
            setHourlyRate(facility.hourlyRate?.toString() || "");
            setAmenities(facility.amenities || []);
            setError(null);

            // Set existing image if available
            if (facility.images && facility.images[0]) {
                setUrl(facility.images[0]);
            } else {
                resetUpload();
            }
        }
    }, [facility, isOpen, setUrl, resetUpload]);
        ...
        try {
            const res = await fetch(`/api/facilities/${facility.id}`, {
                ...
                body: JSON.stringify({
                    ...
                    amenities,
                    images: uploadedUrl ? [uploadedUrl] : (facility.images || [])
                }),
            });
                ...
                <>
                    ...
                    >
                            ...
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-black text-slate-400 uppercase tracking-[0.3em]">
                                        #{facility.id.slice(-6).toUpperCase()} — Adjusting Specifications
                                    </p>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-10 bg-gradient-to-b from-white to-slate-50/30">
                            <form id="edit-facility-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-12">
