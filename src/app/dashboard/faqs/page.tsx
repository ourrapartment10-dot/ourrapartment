...
function AccordionItem({
  ...
  return (
    <div
      className={cn(
        'group border-b border-gray-100 transition-all duration-300 last:border-0',
    ...
    >
      ...
      >
        ...
        <div
          className={cn(
            'ml-4 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full transition-all duration-300',
            isOpen
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="px-5 pb-5 text-sm leading-relaxed whitespace-pre-line text-gray-500">
              {answer.split('**').map((part, i) =>
                i % 2 === 1 ? (
                  <b key={i} className="text-gray-900">
                    {part}
                  </b>
                ) : (
                  part
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 pb-32">
      ...
      <div className="relative mb-8 md:mb-16">
        <div className="absolute top-0 left-1/2 -z-10 h-96 w-96 -translate-x-1/2 -translate-y-12 rounded-full bg-blue-400/10 blur-3xl" />
        <div className="flex flex-col items-center space-y-4 text-center">
          <motion.div
            initial={{ scale: 0.9, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="mb-2 inline-flex items-center gap-2 rounded-full border border-blue-100 bg-blue-50 px-4 py-1.5 text-[10px] font-black tracking-widest text-blue-600 uppercase"
          >
            <Sparkles className="h-3 w-3" />
            Admin Help Center
          </motion.div>
          <h1 className="text-3xl leading-tight font-black tracking-tight text-gray-900 md:text-5xl">
            How can we <span className="text-blue-600">help you?</span>
          </h1>
          <p className="max-w-lg text-xs font-medium text-gray-500 md:text-base">
            Master your community management with our comprehensive guides and
            answers.
          </p>
        </div>
      </div>
      ...
      <div className="sticky top-20 z-30 mb-8 md:mb-12">
        <div className="relative mx-auto max-w-2xl">
          <div className="absolute inset-0 -z-10 rounded-[1.5rem] bg-white/70 shadow-2xl shadow-blue-900/5 backdrop-blur-xl md:rounded-[2.5rem]" />
          <div className="relative flex items-center p-1.5 md:p-2">
            <Search className="absolute left-5 h-4 w-4 text-gray-400 md:left-6 md:h-5 md:w-5" />
            <input
              type="text"
              placeholder="Search help topics..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-transparent py-3 pr-10 pl-12 text-sm font-medium text-gray-900 outline-none placeholder:text-gray-400 md:py-4 md:pr-12 md:pl-14 md:text-base"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-4 rounded-full p-1.5 text-gray-400 transition-colors hover:bg-gray-100 md:right-6"
              >
                <X className="h-4 w-4" />
              </button>
            )}
      ...
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-12">
        ...
        <div className="h-fit space-y-2 lg:sticky lg:top-48 lg:col-span-3">
          {/* Mobile Category Toggle */}
          <button
            onClick={() => setIsCategoryMenuOpen(!isCategoryMenuOpen)}
            className="flex w-full items-center justify-between rounded-2xl border border-gray-100 bg-white px-5 py-4 text-sm font-bold text-gray-900 shadow-sm lg:hidden"
          >
            <div className="flex items-center gap-3">
              ...
              <span>
                {activeTab === 'all'
                  ? 'All Topics'
                  : faqData.find((c) => c.id === activeTab)?.title}
              </span>
            </div>
            <ChevronDown
              className={cn(
          ...
          <AnimatePresence>
            {isCategoryMenuOpen && (
              <motion.div
                ...
                className="space-y-2 overflow-hidden lg:hidden"
              >
                <button
                  onClick={() => {
                    setActiveTab('all');
                ...
                {faqData.map((cat) => (
                  <button
                    ...
                    className={cn(
                      'flex w-full items-center justify-between rounded-2xl px-4 py-3 text-sm font-bold transition-all',
                      activeTab === cat.id
                        ? 'border border-blue-50 bg-white text-blue-600 shadow-md'
                        : 'text-gray-500 hover:bg-white hover:text-gray-900'
                    )}
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={cn(
                          'rounded-lg p-1.5',
                          activeTab === cat.id ? cat.color : 'bg-gray-100'
                        )}
                      >
          ...
          <div className="hidden space-y-2 lg:block">
            <p className="mb-4 ml-4 flex items-center gap-2 text-[10px] font-black tracking-widest text-gray-400 uppercase">
            ...
            {faqData.map((cat) => (
              ...
              >
                ...
                <ArrowRight
                  className={cn(
                    'h-3 w-3 -translate-x-2 opacity-0 transition-all',
                    activeTab === cat.id && 'translate-x-0 opacity-100'
                  )}
                />
              </button>
            ))}
        ...
        {/* Content Area */}
        <div className="space-y-6 md:space-y-10 lg:col-span-9">
          <LayoutGroup>
                ...
                >
                  ...
                  <div className="flex flex-col">
                    <button
                      ...
                      className="group mb-4 flex w-full items-center gap-3 px-2 text-left outline-none lg:cursor-default"
                    >
                      <div
                        className={cn(
                          'flex h-8 w-8 shrink-0 items-center justify-center rounded-xl shadow-sm md:h-10 md:w-10',
                          category.color
                        )}
                      >
                        <category.icon className="h-4 w-4 md:h-5 md:w-5" />
                      </div>
                      <h2 className="flex-1 text-lg font-black text-gray-900 md:text-xl">
                        {category.title}
                      </h2>
                      ...
                      <div
                        className={cn(
                          'flex h-6 w-6 items-center justify-center rounded-full bg-gray-100 text-gray-400 transition-all duration-300 lg:hidden',
                          isExpanded
                            ? 'rotate-180 bg-gray-200 text-gray-600'
                            : ''
                        )}
                      >
                        <ChevronDown className="h-3.5 w-3.5" />
                      ...
                      <div className="ml-2 hidden h-px flex-1 bg-gray-100 lg:block" />
                    </button>
                    ...
                    <div
                      className={cn(
                        ...
                        isExpanded
                          ? 'visible max-h-[2000px] opacity-100'
                          : 'invisible max-h-0 opacity-0 lg:visible lg:max-h-none lg:opacity-100'
                      )}
                    >
                      <div className="divide-y divide-gray-50 overflow-hidden rounded-[1.5rem] border border-gray-100 bg-white shadow-sm md:rounded-[2rem]">
          ...
          {filteredFAQs.length === 0 && (
            <div className="rounded-[2rem] border border-dashed border-gray-200 bg-white py-20 text-center font-medium">
              <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-gray-50 md:h-20 md:w-20">
                <Search className="h-6 w-6 text-gray-300 md:h-8 md:w-8" />
              </div>
              <h3 className="mb-2 text-lg font-bold text-gray-900 md:text-xl">
                No matching help topics
              </h3>
              <p className="mx-auto max-w-xs text-sm text-gray-400 md:text-base">
                Try searching for other keywords.
              </p>
              <button
                onClick={() => setSearchQuery('')}
                className="mt-6 rounded-xl bg-blue-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-blue-200"
              >
                Clear search
              </button>
            </div>
      ...
      <div className="relative mt-20 md:mt-32">
        <div className="absolute inset-0 rounded-[2rem] bg-gradient-to-br from-blue-600 to-indigo-700 shadow-2xl shadow-blue-900/30 md:rounded-[3rem]" />
        <div className="relative flex flex-col items-center justify-between gap-8 p-8 md:flex-row md:gap-10 md:p-16">
          <div className="space-y-3 text-center md:space-y-4 md:text-left">
            <h3 className="text-2xl leading-tight font-black text-white md:text-3xl">
              Still stuck? We're <br />{' '}
              <span className="text-xl text-blue-200 md:text-2xl">
                here to help.
              </span>
            </h3>
          ...
          <div className="flex w-full flex-col gap-3 sm:flex-row md:w-auto md:gap-4">
            <Link
              ...
              className="flex items-center justify-center gap-3 rounded-xl bg-white px-6 py-3.5 text-xs font-black text-blue-600 shadow-xl transition-all hover:scale-[1.02] md:rounded-2xl md:px-8 md:py-4 md:text-sm"
            >
            ...
            <a
              ...
              className="flex items-center justify-center gap-3 rounded-xl border border-white/20 bg-blue-500/30 px-6 py-3.5 text-xs font-black text-white backdrop-blur-md transition-all hover:bg-white/10 md:rounded-2xl md:px-8 md:py-4 md:text-sm"
            >
