    ...
    try {
        ...
        if (config && config.maxProperties > 0) {
            ...
            if (propertyCount >= config.maxProperties) {
                throw new ApiError(403, "Maximum number of properties reached for this community");
            }
        }
        ...
        try {
            ...
            console.error("Failed to trigger admin notifications during signup:", notifWarn);
        }
        ...
        await prisma.refreshToken.create({
            data: {
                token: tokenHash,
                userId: user.id,
                expiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // 7 days matching token expiry
            },
        });
        ...
        response.cookies.set("accessToken", accessToken, { httpOnly: true, path: "/", secure: process.env.NODE_ENV === "production" });
        response.cookies.set("refreshToken", refreshToken, { httpOnly: true, path: "/", secure: process.env.NODE_ENV === "production" });
