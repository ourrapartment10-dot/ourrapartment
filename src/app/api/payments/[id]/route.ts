...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (existingPayment.status === PaymentStatus.COMPLETED) {
            return NextResponse.json({ error: "Completed payments cannot be modified" }, { status: 400 });
        }
        ...
        if (user.role === 'RESIDENT') {
            ...
            if (status && status !== PaymentStatus.PENDING_VERIFICATION) {
                return NextResponse.json({ error: "Residents can only submit for verification" }, { status: 403 });
            }
        ...
        if (user.role === 'ADMIN' || user.role === 'SUPER_ADMIN') {
            const { status, paymentMethod, transactionId, paidDate, amount, type, description, dueDate } = body;
            ...
            if (status === PaymentStatus.COMPLETED && !paidDate && !existingPayment.paidDate) {
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (existingPayment.status === PaymentStatus.COMPLETED) {
            return NextResponse.json({ error: "Completed payments cannot be deleted" }, { status: 400 });
        }
