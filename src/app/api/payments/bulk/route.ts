...
export async function POST(request: NextRequest) {
    try {
        ...
        if (operation === 'create') {
            ...
            // Notify Users
            const uniqueUsers = Array.from(new Set(paymentsData.map(p => p.userId)));
            Promise.allSettled(uniqueUsers.map(async (uid) => {
        ...
        return NextResponse.json({ error: "Operation not supported" }, { status: 400 });
