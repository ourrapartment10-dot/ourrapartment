...
export async function POST(req: NextRequest) {
    try {
        ...
        const { title, content, imageUrl, commentsEnabled, expiresAt, poll: pollData } = body;
            ...
            data: {
                ...
                poll: pollData ? {
                    create: {
                        ...
                        options: {
                            create: pollData.options.map((opt: string) => ({ text: opt }))
                        }
                    }
                } : undefined
            },
        ...
        recipients.forEach(recipient => {
            if (recipient.id !== payload.userId) { // Don't notify self? Optional.
