...
export async function getComplaintStats() {
    ...
    const avgRating = totalRated > 0
        ? ratedComplaints.reduce((acc, curr) => acc + (curr.rating || 0), 0) / totalRated
        : 0;
...
export async function createComplaint(data: {
    ...
    const notificationPromises = admins.map(async (admin) => {
        ...
        await sendPushNotification(admin.id, title, message, link).catch(console.error);
    });
...
export async function getComplaints(filter?: { type?: ComplaintType; status?: ComplaintStatus; myComplaints?: boolean }) {
    const user = await getAuthenticatedUser();
    if (!user) throw new Error("Unauthorized");
    ...
    return prisma.complaint.findMany({
        where: whereClause,
        include: {
            user: { select: { name: true, image: true, property: { select: { flatNumber: true } } } }
        },
...
export async function resolveComplaint(id: string) {
    ...
    await sendPushNotification(complaint.userId, title, message, link).catch(console.error);
...
export async function updateComplaint(id: string, data: {
    ...
    if (existingComplaint.userId !== user.id) throw new Error("Permission denied");
...
export async function submitComplaintFeedback(id: string, rating: number, feedback: string) {
    const user = await getAuthenticatedUser();
    if (!user) throw new Error("Unauthorized");
